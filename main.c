...
int main(int argc, char **argv)
{
    ...
    while(glfwGetKey(GLFW_KEY_ESC) != GLFW_PRESS)
    {
        ...
        if(joystickPresent)
        {
            ...
            float xJoyPos = (pos[0]*32767)/128.0 - 1.0;
            float yJoyPos = (pos[1]*32767)/128.0 + 1.0;
            float zJoyPos = -1*((pos[2]*32767)/128.0 - 1.0);

            if(xJoyPos>.02||xJoyPos<-.02)
            {
                minx += zoom_factor*xJoyPos*deltax*1.0;
                need_view_synch = true;
            }
            if(yJoyPos>.02||yJoyPos<-.02)
            {
                miny += zoom_factor*yJoyPos*deltay*1.0;
                need_view_synch = true;
            }
